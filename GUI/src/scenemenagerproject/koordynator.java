...
public class koordynator implements Initializable {
    ...
    private void dodajlot() {
        ...
        try {
            ...
            CallableStatement cstmt = con.prepareCall("call DODAJ_LOT(?,?,?,?,?,?,?,?,?)");
            cstmt.setInt(1, wynik);
            cstmt.setInt(2, IDSAMOLOT);
            cstmt.setInt(3, 24);
            cstmt.setString(4, start);
            cstmt.setString(5, ladowanie);
            cstmt.setString(6, powrot);
            cstmt.setString(7, "Dostępny");
            cstmt.setString(8, "KorsarzLOT");
            cstmt.setString(9, miejs);
            cstmt.executeQuery();
            con.close();
            System.out.println("Koniec połaczenia");
        } catch (Exception e) {
